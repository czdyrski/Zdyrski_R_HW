...
```{r}
ggplot(num_snp_and_fang_et_al, aes(x= as.numeric(Position))) +
...
```{r}
ggplot(hom_het_added_column, aes(x = Sample_ID, fill = Heterozygotes)) +
...
#Make fifth plot with ggplot, shows Heterozygosity across al the groups using the newly merged file
```{r}
ggplot(hom_het_added_column, aes(x = Group, fill = Heterozygotes)) +
  geom_bar(position = "fill") +
...
ggplot(hom_het_added_column_plot, aes(x = Group, fill = Heterozygotes)) +
  geom_bar(position = "fill") +
